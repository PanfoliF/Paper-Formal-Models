...
\textcolor{red}{We revisit the three puzzels raised by the animal examples in \autoref{sub:problem} (\nameref{sub:problem}):}
...
\textcite{reiterExploringBargainingModel2003} argues: "Classic deterrence theory predicts that as an imbalance of power between two states grows, war becomes more probable. The bargaining model, \textit{on the other hand}, argues that it is not an imbalance of power that causes war, but rather disagreement over the balance of power (and over the likely outcome of war)." \textit{(Emphasis adde.)}

In the light of formal representations of conflic, the first question can be rephrased as follows: why does the animal acts through deterrence in one occasion and thorugh bargaining in the other?

\vspace*{1em}
At first glance, we may thoerise that the animal regrets the attack.

\vspace*{1em}
The decisive variable in this instance may be the element of surprise \parencite{Fearon_1995}.
The two versions of the elephant-human encounter, one involving the shepherd and one involving the tourist, are similar in strategic structure. In the tourist scenario, there is no surprise: the elephant is aware that humans might approach and disturb it, and it knows where to find them.
