...
In the baseline setting agents are represented by perceptions, rationality and behaviour. The former two shape the latter, which is the outcome variable of interest. A change in perceptions corresponds to a change in behaviour.
...
Our outcome variable is the only one we can observe and measure. In the two cases described by \textcite{safinaWordsWhatAnimals2015}, we \textit{observe} the behaviour of elephants and humans. However, we can only \textit{infer} rationality and perceptions.
...
\vspace*{1em}
How do we distinguish between the two agents in the Causal DAG\@? They differ in the realised behavioural outcome. While human behaviour is constant, elephant is different in the two cases (\nameref{sub:problem}), this is why we have one circle for each player.

\vspace*{1em}
We have 4 circles, three of which are constant in the two cases. The fifth one represents elephant's emotions. This variable provides the key source of variation across cases. Thus, it can explain the change in elephant behaviour.
